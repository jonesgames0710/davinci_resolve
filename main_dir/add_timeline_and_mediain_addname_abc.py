...
if project:
    ...
    if root_folder:
        ...
        for i in range(start_bin, bin_count+1):  # 1から100までの範囲でビンに対応するタイムラインを作成
            ...
            timeline_name_a = f"Timeline_{i:03d}_a"  # Timeline_001, Timeline_002,...Timeline_100
            timeline_name_b = f"Timeline_{i:03d}_b"  # Timeline_001, Timeline_002,...Timeline_100
...
#########################################.mp4 の前の数字に基づいてクリップがソートする関数####################################################
            ...
            # タイムラインを追加
            new_timeline_a = media_pool.CreateTimelineFromClips(timeline_name_a,sorted_objects)
            new_timeline_b = media_pool.CreateTimelineFromClips(timeline_name_b,sorted_objects)

            if new_timeline_a:
                print(f"タイムライン '{timeline_name_a}' が '{bin_name}' に正常に追加されました。\n\n")
            else:
                print(f"タイムライン '{timeline_name_a}' の作成に失敗しました。\n\n")
            if new_timeline_b:
                print(f"タイムライン '{timeline_name_b}' が '{bin_name}' に正常に追加されました。\n\n")
            else:
                print(f"タイムライン '{timeline_name_b}' の作成に失敗しました。\n\n")
