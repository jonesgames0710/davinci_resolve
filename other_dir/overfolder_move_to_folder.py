...
# === 設定部分（変数） ===
# リストを作成する検索先フォルダ
source_folder = r"L:\avidolz\saitamaggclub+mojie"
# 検索する文字列
search_string = "over"
# リストを使ってフォルダ名を検索するフォルダ（移動するフォルダがある場所）
search_folder = r"L:\\saitama_apex_avidolz"
# リストに当てはまったフォルダ名の移動先フォルダ
destination_folder = r"L:\saitama_apex_avidolz\saitama"

# === メインロジック ===

def extract_number_from_folder(folder_name, keyword):
    """
    フォルダ名から指定された文字列と番号を抽出する関数。
    例: "Render_avidolz-111over" -> 111
    """
    if keyword in folder_name:
        parts = folder_name.split(keyword)
        if len(parts) > 1:
            num_part = ''.join(filter(str.isdigit, parts[0]))
            return num_part if num_part.isdigit() else None
    return None
...
for folder in os.listdir(source_folder):
    ...
    if os.path.isdir(folder_path):
        number = extract_number_from_folder(folder, search_string)
        if number:
